...
export class StkPipelineStack extends Stack {
  ...
  const buildRole = new iam.Role(this, 'codeBuildDeployAmd64Role', { roleName: "codeBuildDeployAmd64Role",
        ...
        phases: {
          build: {
            commands: [
              ...
              `docker build -t ${this.account}.dkr.ecr.${this.region}.amazonaws.com/${ecrRepoName}:$TAG .`,
              `aws ecr get-login-password --region ${this.region} | docker login --username AWS --password-stdin ${this.account}.dkr.ecr.${this.region}.amazonaws.com/${ecrRepoName}`,
              `docker push ${this.account}.dkr.ecr.${this.region}.amazonaws.com/${ecrRepoName}:$TAG`,
